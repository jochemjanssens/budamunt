...
export default class QRScreen extends React.Component {
  ...
  checkPayments = () => {
    ...
    AsyncStorage.getItem("myToken").then(token => {
      ...
      fetch('http://192.168.1.49:3000/api/transactions', {
        method: 'GET',
        headers
      })
      .then(r => {
  ...
  AsyncStorage.getItem("user").then(user => {
    ...
    AsyncStorage.getItem("myToken").then(token => {
          ...
          AsyncStorage.getItem("muntenId").then(muntenId => {
            fetch(`http://192.168.1.49:3000/api/balances/${muntenId}`, {
                method: "DELETE",
                headers
            })
            .then(d => {
              AsyncStorage.getItem("munten").then(munten => {
                ...
                .then(r => {
                  fetch(`http://192.168.1.49:3000/api/transactions/${transaction._id}`, {
                      method: "DELETE",
                      headers
                  })
                  .then(d => {
