...
export default class QRScreen extends React.Component {
  ...
  checkPayments = () => {
    AsyncStorage.getItem("myToken").then(token => {
      ...
      fetch('http://172.20.66.6:3000/api/transactions?isActive=true', {
        method: 'GET',
        headers
      })
      .then(r => {
  ...
  AsyncStorage.getItem("user").then(user => {
    ...
    AsyncStorage.getItem("myToken").then(token => {
          ...
          AsyncStorage.getItem("muntenId").then(muntenId => {
            fetch(`http://172.20.66.6:3000/api/balances/${muntenId}`, {
                method: "DELETE",
                headers
            })
            .then(d => {
              AsyncStorage.getItem("munten").then(munten => {
                ...
                .then(r => {
                  fetch(`http://172.20.66.6:3000/api/transactions/${transaction._id}`, {
                      method: "DELETE",
                      headers
                  })
                  .then(d => {
